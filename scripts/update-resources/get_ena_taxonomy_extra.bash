...
curl -L https://ftp.ebi.ac.uk/pub/databases/ena/taxonomy/taxonomy.xml.gz \
...
curl -L "https://www.ebi.ac.uk/ena/portal/api/search?result=taxon&query=tax_tree($TAXROOT)&limit=10000000" > resulttaxon.tax_tree$TAXROOT.taxid_desc || exit 1

if [ $(stat -c %s resulttaxon.tax_tree$TAXROOT.taxid_desc) -lt 21 ]; then exit 0; fi
...
gunzip ena-taxonomy.extra.$TAXROOT.jsonl.gz 2>/dev/null || touch ena-taxonomy.extra.$TAXROOT.jsonl
